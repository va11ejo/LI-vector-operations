\page functions "Página das Funções do Programa"
...
operações com vetores e matrizes. A descrição detalhada de cada função é
gerada automaticamente a partir dos comentários Doxygen presentes no
código.
...
- **`matriz_2x10_quadruplo(numbers)`**  
  Constrói uma matriz 2x10 em que a primeira linha é o vetor inicial e a
...
- **`produto_com_ordem_crescente(numbers)`**  
  Gera uma matriz 10x10 a partir do produto entre cada elemento do vetor
  inicial e todos os elementos do mesmo vetor ordenado por ordem
  crescente.

- **`transposta(matriz_10_x_10)`**  
  Calcula e devolve a transposta de uma matriz 10x10 representada como
  lista de listas.
